...
export default function Booking() {
    ...
    const checkAvailableDatesAhead = (startDate: DateRangeProps) => {
        ...
        if (closestNextStartDate)
            setMaxDate(dayjs(closestNextStartDate?.startDate).subtract(1, 'day') as DateRangeProps)
    }
    ...
    const checkAvailableDatesBehind = (endDate: DateRangeProps) => {
        ...
        if (closestPreviousEndDate)
            setMinDate(dayjs(closestPreviousEndDate?.startDate).add(1, 'day') as DateRangeProps)
    }

    const dateRangeChangeHandler = (value: DateRange<DateRangeProps>, context:FieldChangeHandlerContext<(string | null)[]>) => {
